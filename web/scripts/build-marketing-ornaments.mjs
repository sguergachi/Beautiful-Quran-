...
// Geometry shared by both schemes, so CSS can size bands with calc() at
// whatever height a given rule wants.
const shared = [
  `--orn-band-period: ${cover.border.period.toFixed(4)};`,
]

const block = (lines, indent) => lines.map((l) => indent + l).join('\n')
const names = vars.light.map((line) => line.match(/^(--[^:]+):/)[1])
const palette = (scheme) =>
  vars[scheme].map((line, i) => line.replace(names[i], `${names[i]}-${scheme}`))
const usePalette = (scheme) => names.map((name) => `${name}: var(${name}-${scheme});`)
 ...
 * Cover (corner seal and frieze) seed ${COVER_SEED} — Al-Fatihah's
...
${block(palette('light'), '  ')}
${block(palette('dark'), '  ')}
${block(usePalette('light'), '  ')}
}

@media (prefers-color-scheme: dark) {
  :root:not([data-theme]) {
${block(usePalette('dark'), '    ')}
  }
}
